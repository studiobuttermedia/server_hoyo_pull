...
if os.path.isfile("GI/GB/voice.txt") == True:
    file = open("GI/GB/voice.txt", "r")
    content = file.read()
    file.close()
    if content:
        print("Cleaning existing data")
        file = open("GI/GB/voice.txt", "w")
        file.write("")
    else:
        print("No data found in Text file")
else:
    print("Creating file")
    file = open("GI/GB/voice.txt", "a")
...
if r.status_code == 200:
    ...
    for sys1 in content['data']['game']['latest']['voice_packs']:
