...
if os.path.isfile("GI/GB/pre_data.txt") == True:
    file = open("GI/GB/pre_data.txt", "r")
    content = file.read()
    file.close()
    if content:
        print("Cleaning existing data")
        file = open("GI/GB/pre_data.txt", "w")
        file.write("")
        file.close()
    else:
    ...
    print("Creating file")
    file = open("GI/GB/pre_data.txt", "a")
    file.close()
...
if r.status_code == 200:
    ...
    for sys1 in content['data']['pre_download_game']['latest']['segments']:
