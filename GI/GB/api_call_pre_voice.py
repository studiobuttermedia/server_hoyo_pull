...
if os.path.isfile("GI/GB/pre_voice.txt") == True:
    file = open("GI/GB/pre_voice.txt", "r")
    content = file.read()
    file.close()
    if content:
        print("Cleaning existing data")
        file = open("GI/GB/pre_voice.txt", "w")
        file.write("")
        file.close()
    else:
    ...
    print("Creating file")
    file = open("GI/GB/pre_voice.txt", "a")
    file.close()
...
if r.status_code == 200:
    ...
    for sys1 in content['data']['pre_download_game']['latest']['voice_packs']:
